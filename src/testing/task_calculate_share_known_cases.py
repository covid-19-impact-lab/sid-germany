...
def _calculate_share_known_cases(df):
    ...
    end_until_feb = pd.date_range(
        start=share_known_cases.index.max(), end="2021-02-01", closed="right"
    )
    ...
    assert (
        share_known_cases.index == pd.date_range(start="2020-01-01", end="2021-02-01")
    ).all()
